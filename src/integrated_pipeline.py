...
coords_list = [-120, -88, 534, 180, 0, -92.5]  # 這裡是手臂的座標，請根據實際情況修改
...
time.sleep(1)  # 等待手臂移動完成
#-120 -88 534 180 0 -92.5
print(f"手臂移動完成，座標：{coords_list}")
sock.close()
...
#IMG_PATH = r"C:\Users\Gillion-BennyWinNB\Desktop\2025HIWIN_poolball\take_photo_pool\extra_photo_019.png"#改成圖片的絕對路徑
IMG_PATH=None
take="take_images"
while True:
    ret, frame = cap.read()
...
serial_port = "COM3"  # 替換為你的 COM 埠號

port = 'COM5'  # 請依實際情況修改
baudrate = 9600  # 與 Arduino 程式中 Serial.begin() 的速率相同

try:
    # 建立串列埠連線
    ser = serial.Serial(port, baudrate, timeout=1)
    # Arduino 在開啟序列埠時通常會重置，等待一小段時間
    time.sleep(2)

    # 要傳送的資料
    data = b'1'  # bytes 類型；若要傳送字串，可用 '1'.encode()
...
sock.sendall(str(coords_list).encode())
  # 等待手臂移動完成
#-120 -88 534 180 0 -92.5
print(f"手臂移動回到home，座標：{coords_list}")
sock.close()
